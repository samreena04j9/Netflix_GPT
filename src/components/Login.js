    ...
    if (!isSignInForm) {
      ...
      createUserWithEmailAndPassword( auth, email.current.value, password.current.value )
        .then((userCredential) => {
          ...
          updateProfile(user, {
            displayName:name.current.value, photoURL: "https://avatars.githubusercontent.com/u/61137081?v=4"
          }).then(() => {
    ...
    <div>
      ...
      >
        ...
        <input
          ...
          className="p-4 my-4 w-full bg-gray-700"
        />
        <p className="text-bold text-red-500 text-lg py-2">{errorMessage}</p>
        <button
